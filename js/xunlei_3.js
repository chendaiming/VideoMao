...
if ($(this).attr("checked")){ // 全选 
	$("#copyurl").val('');	 
     $("input[name='copyaddr"+num+"']").attr("checked", 'true');
	  var str='';
	 $("input[name='copyaddr"+num+"']:checked").each(function () {
		 var val=$(this).val()+"###";
		 str+=val;
	 });
	$("#copyurl").val(str);
} else { // 取消全选 
         $("input[name='copyaddr"+num+"']").removeAttr("checked");
		 $("#copyurl").val('');		
} 
});
//单击复选框
...
$(".dcopyzclip").zclip({
        ...
        copy: function(){
			...
			reg=new RegExp("\\###","g");
			var str=copyurl.replace(reg, "\r\n");
            return str;
        },beforeCopy: function(){//复制成功
		
        },afterCopy: function(){//复制成功
		var copyurl=$("#copyurl").val();
		if(copyurl==''){
          alert('请选择要复制的资源'+copyurl);
		  return false;
		}
          alert('已经全部复制到粘贴板');
        }
